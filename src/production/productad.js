...
class ProductAdProducer {
    ...
    async buildPrompt(job) {
        ...
        if (!hook && llm.hasDeepSeek()) {
            ...
            try {
                const reply = await llm.chatDeepSeek([
                    { role: 'system', content: `You write short, punchy UGC-style short-form video ad concepts. Reply with ONE vivid 1-2 sentence shot description in ${lang}. No preamble, no quotes.` },
        ...
        return `${hook} Vertical 9:16, mobile-first, high energy, suitable for TikTok/Reels/Shorts. Feature the product prominently and clearly.`;
    }
}
